...
class ExtractionPlateBlockEntity(
	pos: BlockPos,
	blockState: BlockState
) : BlockEntity(ModBlockEntityTypes.EXTRACTION_PLATE.get(), pos, blockState) {

	private fun tick() {
		val level = level as? ServerLevel ?: return
		if (level.gameTime % TRANSFER_INTERVAL != 0L) return

		val inputDirection = blockState.getValue(ExtractionPlateBlock.INPUT)
		val outputDirection = blockState.getValue(ExtractionPlateBlock.OUTPUT)
	...
	private fun dropItem(level: ServerLevel, outputDirection: net.minecraft.core.Direction, stack: ItemStack) {
		val center = worldPosition.center
		val x = center.x + outputDirection.stepX * 0.65
		val y = worldPosition.y + 0.1
		val z = center.z + outputDirection.stepZ * 0.65

		val item = ItemEntity(level, x, y, z, stack)
	...
	companion object {
		...
		fun tick(
			...
			pos: BlockPos,
			state: BlockState,
			blockEntity: ExtractionPlateBlockEntity
		) {
