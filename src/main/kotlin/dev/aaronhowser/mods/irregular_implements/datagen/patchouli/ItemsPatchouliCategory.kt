...
object ItemsPatchouliCategory {
	...
	lateinit var spectreHelmet: PatchouliBookEntry
	lateinit var waterWalkingBoots: PatchouliBookEntry
	lateinit var bean: PatchouliBookEntry
	lateinit var biomePainter: PatchouliBookEntry
	lateinit var blazeAndSteel: PatchouliBookEntry
	lateinit var blockMover: PatchouliBookEntry
	lateinit var blockReplacer: PatchouliBookEntry
	lateinit var bottleOfAir: PatchouliBookEntry
	lateinit var chunkAnalyzer: PatchouliBookEntry
	lateinit var diviningRod: PatchouliBookEntry
	lateinit var dropFilter: PatchouliBookEntry
	lateinit var ectoplasm: PatchouliBookEntry
	lateinit var emeraldCompass: PatchouliBookEntry
	lateinit var enderBucket: PatchouliBookEntry
	lateinit var enderLetter: PatchouliBookEntry
	lateinit var escapeRope: PatchouliBookEntry
	lateinit var evilTear: PatchouliBookEntry
	lateinit var entityFilter: PatchouliBookEntry
	lateinit var itemFilter: PatchouliBookEntry
	lateinit var locationFilter: PatchouliBookEntry
	lateinit var playerFilter: PatchouliBookEntry
	lateinit var flooPouch: PatchouliBookEntry
	lateinit var flooPowder: PatchouliBookEntry
	lateinit var flooSign: PatchouliBookEntry
	lateinit var flooToken: PatchouliBookEntry
	lateinit var goldenCompass: PatchouliBookEntry
	lateinit var goldenEgg: PatchouliBookEntry
	lateinit var grassSeeds: PatchouliBookEntry
	lateinit var imbueFire: PatchouliBookEntry
	lateinit var lavaCharm: PatchouliBookEntry
	lateinit var lotusBlossom: PatchouliBookEntry
	lateinit var luminousPowder: PatchouliBookEntry
	lateinit var magicBean: PatchouliBookEntry
	lateinit var obsidianSkull: PatchouliBookEntry
	lateinit var portableEnderBridge: PatchouliBookEntry
	lateinit var portkey: PatchouliBookEntry
	lateinit var redstoneActivator: PatchouliBookEntry
	lateinit var redstoneRemote: PatchouliBookEntry
	lateinit var redstoneTool: PatchouliBookEntry
	lateinit var spectreAnchor: PatchouliBookEntry
	lateinit var spectreCharger: PatchouliBookEntry
	lateinit var spectreIlluminator: PatchouliBookEntry
	lateinit var spectreKey: PatchouliBookEntry
	lateinit var spectreSword: PatchouliBookEntry
	lateinit var stableEnderPearl: PatchouliBookEntry
	lateinit var summoningPendulum: PatchouliBookEntry
	lateinit var superLubricantTincture: PatchouliBookEntry
	lateinit var voidStone: PatchouliBookEntry
	lateinit var weatherEgg: PatchouliBookEntry
	lateinit var whiteStone: PatchouliBookEntry
	...
	private fun addEntries(book: PatchouliBook) {
		lavaWaders = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Lava Waders")} combine the effects of the ${internalLink(obsidianWaterWalkingBoots, "Obsidian Water Walking Boots")} and the ${internalLink(lavaCharm, "Lava Charm")}, as well as allowing you to walk on Lava.",
		...
		magicHood = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Magic Hood")}, when worn, will hide your nameplate and potion particles.",
		...
		obsidianWaterWalkingBoots = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Obsidian Water Walking Boots")} combine the effects of the ${internalLink(waterWalkingBoots, "Water Walking Boots")} and the ${internalLink(obsidianSkull, "Obsidian Skull")}.",
					"Get them by combining the two in an Anvil."
				)
			)
		...
		spectreHelmet = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Spectre Armor")} is equivalent to Diamond, with higher durability and enchantability.",
					"Wearing a full set also makes you slightly transparent!"
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_helmet"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_chestplate"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_leggings"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_boots"))
		}
		...
		waterWalkingBoots = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Water Walking Boots")} allow you to walk on water. Wow!",
		...
		bean = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Beans")} can be found growing around the overworld in any biome that is neither hot, cold, dry, nor sparse.",
		...
		biomePainter = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Biome Capsule")} and ${major("Biome Painter")} are used together to allow you to change the biome of a location.",
			...
			craftingPage(ResourceLocation.parse("irregular_implements:biome_painter"))

			craftingPage(ResourceLocation.parse("irregular_implements:biome_capsule"))
		}


		blazeAndSteel = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Blaze and Steel")} lights a much more aggressive fire, which burns and spreads much faster."
				)
			)
		}


		blockMover = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Block Mover")} can pick up and place down a single block, including its block entity.",
		...
		blockReplacer = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Block Replacer")} allows you to quickly replace blocks in the world with blocks stored in the item.",
		...
		bottleOfAir = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Bottle of Air")} can be \"drunk\" to refill your air supply when underwater.",
		...
		chunkAnalyzer = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Chunk Analyzer")} can be used to see the blocks that make up the chunk you're currently standing in.",
		...
		dropFilter = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Drop Filter")} can hold an ${internalLink(itemFilter, "Item Filter")}, and will prevent items matching the filter from entering your inventory.",
					"The ${major("Voiding Drop Filter")} does the same, but instead deletes the items matching the filter entirely."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:drop_filter"))

			craftingPage(ResourceLocation.parse("irregular_implements:voiding_drop_filter"))
		}


		ectoplasm = book.entry(
			category = bookCategory,
			saveName = "ectoplasm",
			name = "Ectoplasm",
		...
		emeraldCompass = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Emerald Compass")} aims at a bound player, assuming they're online and in the same dimension as you. There are two ways to bind the Compass to a player:",
		...
		enderBucket = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Ender Bucket")} can be used on a flowing fluid, and it will gather the nearest fluid source from a distance.",
					"The ${major("Reinforced Ender Bucket")} works the same, but can also store up to 10 fluid sources at once."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:ender_bucket"))

			craftingPage(ResourceLocation.parse("irregular_implements:reinforced_ender_bucket"))
		}


		enderLetter = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Ender Letter")} allows you to send items to other players more easily. It can store up to 9 stacks inside itself.",
					"In its GUI, type the recipient's username. Then, simply use it on any ${internalLink(BlocksPatchouliCategory.enderMailbox, "Ender Mailbox")}. If the recipient's Mailbox is not full, it will be placed inside it."
				)
			)
		}


		escapeRope = book.entry(
			category = bookCategory,
		...
		evilTear = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Evil Tear")} can be used to make an Artificial End Portal.",
		...
		entityFilter = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Entity Filter")} allows you to filter based on entity type.",
		...
		itemFilter = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Item Filter")} has a list of up to 9 items, and can serve as either a Whitelist or a Blacklist. Toggle this using the button on the far right of the GUI.",
		...
		locationFilter = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Location Filter")} allows you to set a location, which can then be used by other items and blocks to reference that location.",
		...
		playerFilter = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Player Filter")} allows you to filter based on a specific player.",
		...
		flooPouch = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Floo Pouch")} can store up to 128 ${internalLink(flooPowder, "Floo Powder")}.",
		...
		flooPowder = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Floo Powder")} is the fuel that allows you to use Floo teleportation. It must be held in-hand when the destination is said in chat."
				)
			)
		}
		...
		flooSign = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Floo Signs")} are used to create Floo Fireplaces. Using the Sign on Bricks placed in the world will convert them all to Floo Bricks.",
		...
		flooToken = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Floo Tokens")} allow you to use Floo teleportation from anywhere in the world.",
		...
		goldenCompass = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Golden Compass")} aims at a bound location, assuming it's in the same dimension as the user.",
		...
		goldenEgg = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Golden Eggs")} spawn Golden Chickens when thrown. Instead of laying Eggs, Golden Chickens lay Gold Ingots.",
		...
		grassSeeds = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Grass Seeds")} can be used on dirt blocks to turn them into Grass.",
					"${major("Colored Grass Seeds")} do the same, but make Colored Grass."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:grass_seeds"))

			craftingPage(ResourceLocation.parse("irregular_implements:grass_seeds_red"))
		}


		imbueFire = book.entry(
			category = bookCategory,
			saveName = "imbue_fire",
			name = "Imbues",
			...
			textPage(
				text = bookText(
					"${major("Imbues")} are special potions that are created in the ${internalLink(BlocksPatchouliCategory.imbuingStation, "Imbuing Station")}. They provide powerful long-lasting effects when consumed.",
			...
			textPage(
				text = bookText(
					"${major("Available Imbues")}",
					"- ${major("Fire Imbue")}: Melee attacks set targets on fire.\n- ${major("Poison Imbue")}: Melee attacks inflict Poison II.\n- ${major("Wither Imbue")}: Melee attacks inflict Wither II.\n- ${major("Experience Imbue")}: Double experience from slain mobs.\n- ${major("Spectre Imbue")}: 10% chance to completely negate incoming damage.\n- ${major("Collapse Imbue")}: Melee attacks inflict ${internalLink(BlocksPatchouliCategory.sakanadeSpores, "Collapse")}."
				)
			)
		}


		lavaCharm = book.entry(
			category = bookCategory,
			saveName = "lava_charm",
			name = "Lava Charm",
			...
			textPage(
				text = bookText(
					"The ${major("Lava Charm")} can be worn as a Curio, and adds a temporary lava shield. It is visible above your armor bar.",
		...
		lotusBlossom = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Lotus Blossoms")} can be eaten to give the player a handful of experience. Sneaking will consume the entire stack.",
		...
		luminousPowder = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Luminous Powder")} can be crafted with any item to make it glow in the dark.",
		...
		magicBean = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Lesser Magic Bean")}",
					"${major("Lesser Magic Beans")} can be planted to make a Bean Stalk that grows upwards until it's blocked by something. It can be climbed like a ladder!",
					"${major("Magic Bean")}",
					"The non-lesser ${major("Magic Bean")} is a much more powerful version. They grow much, much faster, and will break (almost) any block in their way."
				)
			)

			textPage(
				text = bookText(
					"Once it reaches the top of the world (or if it wouldn't be able to grow further), it will place a ${major("Bean Pod")} at its top. Break it open to collect some rare loot!",
					"The Magic Bean cannot be crafted. Instead, it can be found in dungeon loot and ${internalLink(BlocksPatchouliCategory.natureCore, "Nature Core")} chests."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:lesser_magic_bean"))
		}


		obsidianSkull = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Obsidian Skull")} has a chance of negating fire damage while in your inventory.",
			...
			textPage(
				text = bookText(
					"`` (damage ^ 3) / 100 ``",
					"The ${major("Obsidian Skull Ring")} does the exact same, but can be worn in a Curio slot."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:obsidian_skull_ring"))
		}


		portableEnderBridge = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Portable Ender Bridge")} can be used to teleport to ${internalLink(BlocksPatchouliCategory.enderBridge, "Ender Anchors")}.",
		...
		portkey = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Portkey")} is used to teleport whoever picks it up.",
		...
		redstoneActivator = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Redstone Activator")} will provide a short redstone pulse when used on a block.",
		...
		redstoneRemote = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Redstone Remote")} acts like the ${internalLink(redstoneActivator, "Redstone Activator")}, but can be used from a distance.",
		...
		redstoneTool = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Redstone Tool")} is used to link certain wireless redstone blocks to other locations.",
			...
			textPage(
				text = bookText(
					"The Redstone Tool can be used on the following blocks:\n- ${internalLink(BlocksPatchouliCategory.basicRedstoneInterface, "Basic Redstone Interface")}\n- ${internalLink(BlocksPatchouliCategory.redstoneObserver, "Redstone Observer")}"
				)
			)
		}


		spectreAnchor = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Spectre Anchor")} can be crafted together with any item to allow you to retain it after death.",
		...
		spectreCharger = book.entry(
			...
			textPage(
				text = bookText(
					"${major("Spectre Chargers")} are used to charge items in your inventory with FE from your ${internalLink(BlocksPatchouliCategory.spectreEnergyInjector, "Spectre Energy Buffer")}.",
			...
			textPage(
				text = bookText(
					"Each tier charges faster than the last. The ${major("Genesis Spectre Charger")} is creative only, and does not use your Spectre Energy Buffer's energy."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_charger_basic"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_charger_redstone"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_charger_ender"))
		}


		spectreIlluminator = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Spectre Illuminator")} can be used on a block to fully light up the chunk it's in.",
			...
			textPage(
				text = bookText(
					"This can be undone by either right-clicking the Illuminator entity, or by using ${major("Blackout Powder")} in the chunk."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_illuminator"))

			craftingPage(ResourceLocation.parse("irregular_implements:blackout_powder"))
		}


		spectreKey = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Spectre Key")} can be used to teleport to a private room in another dimension, unique to each player.",
		...
		spectreSword = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Spectre Pickaxe")}, ${major("Spectre Axe")}, and ${major("Spectre Shovel")} are comparable to their Diamond counterparts, with higher durability and enchantability.",
					"They increase your block interaction range by 3 blocks."
				)
			)

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_sword"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_pickaxe"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_axe"))

			craftingPage(ResourceLocation.parse("irregular_implements:spectre_shovel"))
		}


		stableEnderPearl = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Stable Ender Pearl")} is used to teleport a bound player in a more controlled way.",
		...
		summoningPendulum = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Summoning Pendulum")} can be used to store up to 5 (by default) entities, and place them back down later.",
		...
		superLubricantTincture = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"The ${major("Super Lubricant Tincture")} can be crafted with any boot item to make them completely negate friction when worn, while below a certain speed.",
		...
		voidStone = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("Void Stone")} will delete any items inserted into its GUI."
				)
			)
		}


		weatherEgg = book.entry(
			category = bookCategory,
			...
			textPage(
				text = bookText(
					"${major("Weather Eggs")} change the weather when thrown.",
		...
		whiteStone = book.entry(
			...
			textPage(
				text = bookText(
					"The ${major("White Stone")} has the power to prevent your death once while charged. Doing so fully discharges the item.",
